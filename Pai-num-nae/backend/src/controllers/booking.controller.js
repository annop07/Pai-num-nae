...
const uploadPaymentProof = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    const result = await uploadToCloudinary(req.file.buffer, "painamnae/slips");
  ...
  // Handle data from validate middleware (preprocessed) 
  // or handle raw if validate is skipped/fails
  const isReceiptRequested = req.body.isReceiptRequested;
  const receiptDetails = req.body.receiptDetails;

  const updated = await bookingService.uploadPaymentProof(id, passengerId, {
    paymentProofUrl,
    isReceiptRequested,
    receiptDetails
  });
