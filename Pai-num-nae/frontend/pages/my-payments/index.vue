                    ...
                    <div v-for="payment in filteredPayments" :key="payment.id" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col sm:flex-row justify-between relative transition-all duration-200 hover:shadow-md">
                        ...
                        <div class="mt-6 sm:mt-0 sm:ml-4 flex flex-col justify-between items-start sm:items-end min-w-[120px]">
                            ...
                            <div class="w-full sm:w-auto h-9 flex items-end">
                                <NuxtLink v-if="payment.status === 'CONFIRMED' && payment.isReceiptRequested !== false" 
                                    :to="`/my-payments/document/${payment.id}`"
                                    class="w-full sm:w-auto px-4 py-1.5 text-sm font-bold text-green-600 border border-green-500 rounded-md hover:bg-green-50 transition-colors focus:ring-2 focus:ring-green-200 text-center flex items-center justify-center">
                                    ดูใบกำกับภาษี/ใบสำคัญรับเงิน
                                </NuxtLink>
    ...
    try {
        ...
        payments.value = bookings.filter(b => b.status === 'CONFIRMED' || b.status === 'COMPLETED').map(b => {
             ...
             return {
                 ...
                 // Display status
                 status: b.paymentStatus || (Math.random() > 0.5 ? 'CONFIRMED' : 'PENDING'),
                 isReceiptRequested: b.isReceiptRequested ?? true, // Default to true for testing if missing
