          ...
          <div class="modal-scroll-area">
            ...
            <div class="evidence-section" v-if="selectedIncident.attachments?.length">
              ...
              <div class="gallery-grid">
                <div v-for="(file, idx) in selectedIncident.attachments" :key="idx" class="img-wrapper">
                  <video v-if="isVideo(file)" :src="file" controls class="evidence-video"></video>
                  <img v-else :src="file" alt="evidence">
                </div>
              </div>
            </div>

            <div v-if="selectedIncident.newAttachments?.length" class="evidence-section">
              <label>Newly Uploaded</label>
              <div class="preview-grid">
                <div v-for="(file, idx) in selectedIncident.newAttachments" :key="idx" class="preview-box">
            ...
            <div class="map-section" v-if="selectedIncident.location">
              ...
              <div class="map-box">
                <iframe 
                  width="100%" height="180" frameborder="0" loading="lazy"
...
.kpi-card { background: white; padding: 24px; border-radius: 16px; display: flex; justify-content: space-between; align-items: center; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); transition: all 0.3s ease; border: 1px solid #e2e8f0; }
...
.smooth-table th { background: #f8fafc; padding: 16px 20px; text-align: left; font-size: 13px; font-weight: 600; color: #475569; text-transform: uppercase; border-bottom: 1px solid #e2e8f0; }
.smooth-table td { padding: 18px 20px; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
.table-row:hover { background-color: #f8fafc; }

/* Status & Badges */
.status-pill { border: none; background: #f1f5f9; font-weight: 600; font-size: 12px; cursor: pointer; padding: 6px 12px; border-radius: 20px; }
...
.mini-badge { padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 700; margin-right: 8px; text-transform: uppercase; }
...
.desc-text { background: #f8fafc; padding: 16px; border-radius: 12px; line-height: 1.6; border: 1px solid #e2e8f0; }
.gallery-grid, .preview-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 12px; margin-top: 10px; }
.img-wrapper img, .preview-box img, .preview-box video, .evidence-video { width: 100%; height: 100px; object-fit: cover; border-radius: 12px; border: 1px solid #e2e8f0; }
textarea { width: 100%; border-radius: 12px; border: 1px solid #cbd5e1; padding: 12px; resize: none; min-height: 80px; margin-top: 8px; }

/* Buttons */
.modal-foot { padding: 20px 24px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center; }
.btn-submit { background: #3b82f6; color: white; border: none; padding: 10px 20px; border-radius: 10px; font-weight: 600; cursor: pointer; }
.btn-chat-alt { background: #0ea5e9; color: white; border: none; padding: 10px 20px; border-radius: 10px; font-weight: 600; cursor: pointer; }
.btn-ghost { background: white; color: #64748b; border: 1px solid #cbd5e1; padding: 10px 20px; border-radius: 10px; font-weight: 600; cursor: pointer; }
...
const startChat = (incident) => { alert(`Chatting with ${incident.reporter}`) }
const updateStatus = (incident) => { console.log("Status sync:", incident.id, incident.status) }
...
const resolveIncident = () => {
  ...
  if (index !== -1) {
    ...
    incidents.value[index].resolution = selectedIncident.value.resolution
  }
  closeDetail()
}

const fetchData = () => { console.log("Data Refreshed") }
</script>