<template>
    <div >
                    ...
                    <div>
                        ...
                        <div class="pt-8 mt-8 border-t border-gray-200">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="flex flex-col items-center justify-between p-4 bg-white border border-gray-200 rounded-md sm:flex-row shadow-sm">
                                    <span class="mb-3 text-sm text-gray-600 sm:mb-0">เมื่อคุณลบบัญชี จะไม่มีวิธีย้อนกลับได้ โปรดตรวจสอบให้แน่ใจ</span>
                                    <button type="button" @click="confirmDeleteAccount" class="px-4 py-2 text-sm font-bold text-red-600 transition-colors bg-white border border-gray-200 rounded-md hover:bg-red-50 flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-red-500 whitespace-nowrap">
                                        ลบบัญชี
                                    </button>
                                </div>
                            </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center px-4 bg-gray-500/30 backdrop-blur-md">
            <div class="w-full max-w-md overflow-hidden bg-white shadow-xl rounded-xl">
                <div class="p-8">
                    ...
                    <div class="p-4 mb-6 border border-gray-200 rounded-lg bg-gray-50">
                        <p class="mb-2 text-sm font-bold text-gray-800">เราจะลบบัญชีของคุณอย่างถาวรและข้อมูลที่เกี่ยวข้องทั้งหมด:</p>
                    ...
                    <div class="mb-6">
                        <p class="mb-2 text-sm font-bold text-gray-800">เพื่อยืนยัน โปรดพิมพ์ "<span class="text-red-600">delete my account</span>" ด้านล่าง:</p>
                        <input type="text" v-model="deleteConfirmText" placeholder="delete my account" class="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500" />
                    </div>
                    
                    <div class="flex gap-4">
                        <button @click="closeDeleteModal" type="button" class="w-1/2 px-4 py-3 text-sm font-bold text-white transition-colors bg-gray-300 rounded-md hover:bg-gray-400 focus:outline-none">
                            ยกเลิก
                        </button>
                        <button @click="executeDeleteAccount" type="button" :disabled="deleteConfirmText !== 'delete my account' || isDeleting" class="flex items-center justify-center w-1/2 px-4 py-3 text-sm font-bold text-white transition-colors bg-red-600 rounded-md hover:bg-red-700 focus:outline-none disabled:bg-red-400 disabled:cursor-not-allowed">
                            <svg v-if="isDeleting" class="w-5 h-5 mr-2 text-white animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const executeDeleteAccount = async () => {
    ...
    try {
        // TODO: สวม API ลบบัญชีตัวจริงตรงนี้
        // await $api('/users/me', { method: 'DELETE' });
        
        // จำลองการเรียก API
        await new Promise(resolve => setTimeout(resolve, 800));
        
        toast.success('ลบบัญชีสำเร็จ', 'บัญชีของคุณถูกลบแล้ว');
        closeDeleteModal();
        logout(); // คืนค่า Auth และกลับไปหน้า Home
    } catch (err) {
        toast.error('เกิดข้อผิดพลาด', err.message || 'ไม่สามารถลบบัญชีได้');
    } finally {
