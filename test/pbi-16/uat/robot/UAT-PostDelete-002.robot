...
PD-03 - Deleted User Data Still Exists Before Purge Job
    ...
    ${result}=    Run Process    node    ${SCRIPTS_DIR}/set_deleted_at_90days.js    TestPassengerDelete@gmail.com
    Log    STDOUT: ${result.stdout}
    Log    STDERR: ${result.stderr}
    Should Contain    ${result.stdout}    UPDATED:1

    # Step 2: Verify user still exists in DB
    ${check}=    Run Process    node    ${SCRIPTS_DIR}/check_user_exists.js    TestPassengerDelete@gmail.com
    Log    STDOUT: ${check.stdout}
    Log    STDERR: ${check.stderr}
    Should Contain    ${check.stdout}    USER_EXISTS:TestPassengerDelete@gmail.com
...
PD-05 - Related Data Handled After Purge
    ...
    ${check}=    Run Process    node    ${SCRIPTS_DIR}/check_related_data.js    TestPassengerDelete@gmail.com
    Log    STDOUT: ${check.stdout}
    Log    STDERR: ${check.stderr}
